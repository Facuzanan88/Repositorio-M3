...
function problemA() {
  ...
  // promise version
  // ???
  const prom1 = promisifiedReadFile('poem-two/stanza-01.txt').then(stanza => blue(stanza));
  const prom2 = promisifiedReadFile('poem-two/stanza-02.txt').then(stanza => blue(stanza));

  Promise.all([prom1, prom2]).then(() => console.log('-- A. callback version done --'))
}
...
function problemB() {
  ...
  // promise version
  // ???
  const promise = filenames.map(filename => promisifiedReadFile(filename).then(stanza => blue(stanza)))

  Promise.all(promise).then(() => console.log('-- B. callback version done --'))
}
