...
function problemC() {
  ...
  /*  readFile('poem-one/stanza-02.txt', function (err, stanza2) {
     ...
     blue(stanza2);console.log('-- C. callback version (stanza two) --');
     blue(stanza2);
  ...
  promisifiedReadFile('poem-one/stanza-02.txt')
    .then(stanza2 => {
      console.log('-- C. callback version (stanza two) --');
      blue(stanza2)
      return promisifiedReadFile('poem-one/stanza-03.txt'); // si no retorno nada nunca le va a llegar la data al '.then'
    })
    .then(stanza3 => {
      console.log('-- C. callback version (stanza three) --');
      blue(stanza3);
...
function problemD() {
  ...
  promisifiedReadFile('poem-one/wrong-file-name.txt')
    .then(stanza4 => {
      console.log('-- D. callback version (stanza four) --');          // .then(stanza4 => blue(stanza4), err => magenta(new Error(err)))
      blue(stanza4)
    })
